...
import { FileText, ArrowLeft, Users, Trophy, TrendingUp } from "lucide-react";
...
export default function ResultsClient({ paperId }: { paperId: string }) {
  ...
  const fetchResults = async () => {
    try {
      ...
      // Fetch paper details
      const paperResponse = await axios.get(`${API_URL}/papers/${paperId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setPaperTitle(paperResponse.data.paper?.title || "Paper Results");

      // Fetch results
      const resultsResponse = await axios.get(`${API_URL}/papers/${paperId}/results`, {
        headers: { Authorization: `Bearer ${token}` },
      });
