...
import { DashboardSkeleton } from "@/components/teacher/skeletons/DashboardSkeleton";
...
export function TeacherDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Performance Overview and Engagement */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-[#1a2332] rounded-2xl shadow-xl border border-[#2d3748] p-6">
          ...
          <div className="space-y-4 mb-6">
            {data.performanceDistribution && data.performanceDistribution.length > 0 ? (
              data.performanceDistribution.map((item, index) => {
                let color = "bg-gray-500";
                if (item.label.includes("Excellent")) color = "bg-green-500";
                else if (item.label.includes("Good")) color = "bg-blue-500";
                else if (item.label.includes("Average")) color = "bg-orange-500";
                else if (item.label.includes("Below")) color = "bg-red-500";

                return (
                  <div key={index}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium text-gray-300">{item.label}</span>
                      <span className="text-sm font-bold text-white">{item.percentage.toFixed(1)}%</span>
                    </div>
              ...
              <div className="text-center py-8 text-gray-400">No performance data available yet</div>
            )}
          </div>
        ...
        {/* Student Engagement - Takes 1 column */}
        <div className="bg-[#1a2332] rounded-2xl shadow-xl border border-[#2d3748] p-6">
          ...
          <div className="relative w-48 h-48 mx-auto mb-6">
            <svg viewBox="0 0 100 100" className="transform -rotate-90">
              ...
              <circle
                ...
                stroke="#10b981"
                strokeWidth="20"
                strokeDasharray={`${(data.stats.activeStudents / (data.stats.totalStudents || 1)) * 251.2} 251.2`}
                strokeDashoffset="0"
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <p className="text-4xl font-bold text-white">{data.stats.totalStudents}</p>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 rounded-lg bg-[#243447]">
              ...
              <span className="text-lg font-bold text-white">{data.stats.activeStudents}</span>
            </div>
            <div className="flex items-center justify-between p-3 rounded-lg bg-[#243447]">
              <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-bold text-white">{data.stats.inactiveStudents}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {data.recentPapers.slice(0, 3).map((paper) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-400">
                    ...
                    <span className={`flex items-center gap-1 ${paper.completionRate >= 50 ? 'text-green-400' : 'text-orange-400'
                      }`}>
              ...
              {data.students.slice(0, 3).map((student, index) => (
                ...
                >
                  <div className="relative">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-lg ${index === 0 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600' :
                  ...
                  <div className={`text-sm font-medium flex items-center gap-1 ${student.averageScore >= 80 ? 'text-green-400' :
        ...
        {/* Upcoming Deadlines */}
        <div className="bg-[#1a2332] rounded-2xl shadow-xl border border-[#2d3748] p-6 hover:shadow-2xl transition-all duration-300 lg:col-span-3">
          ...
          {data?.recentPapers && data.recentPapers.filter(p => new Date(p.deadline) > new Date()).length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {data.recentPapers
                ...
                .map((paper) => (
                  <div
                    key={paper._id}
                    className="p-4 rounded-xl bg-[#243447] hover:bg-[#2d3f54] transition-all cursor-pointer group border border-gray-700"
                    onClick={() => router.push(`/teacher/papers/${paper._id}`)}
                  >
                    <div className="flex items-start justify-between mb-3">
                      <h4 className="font-medium text-white group-hover:text-red-400 transition-colors truncate pr-2">{paper.title}</h4>
                      <div className="px-2 py-1 rounded bg-red-500/20 text-red-400 text-xs font-medium whitespace-nowrap">
