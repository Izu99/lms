          ...
          <div className="theme-bg-primary rounded-lg shadow-sm theme-border p-4 sm:p-6">
            ...
            <div className="space-y-3">
              {data.recentVideos.length > 0 ? data.recentVideos.map((video) => (
                ...
                <p className="theme-text-secondary text-center py-8">No recent videos available</p>
              )}
            </div>
          </div>
            ...
            <div className="space-y-3">
              {data.availablePapers.length > 0 ? data.availablePapers.map((paper) => (
                <div key={paper._id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                  ...
                  <div className="flex-shrink-0">
                    {paper.isCompleted ? (
                      <span className="inline-block text-green-600 dark:text-green-400 text-sm font-medium bg-green-50 dark:bg-green-900/50 px-2 py-1 rounded">
                        Completed ({paper.percentage}%)
                      </span>
                    ) : (
                      <span className="inline-block text-blue-600 dark:text-blue-400 text-sm font-medium bg-blue-50 dark:bg-blue-900/50 px-2 py-1 rounded">
                        Available
                      </span>
                    )}
                  </div>
                ...
                <p className="theme-text-secondary text-center py-8">No papers available</p>
              )}
            </div>
          </div>
        </div>
