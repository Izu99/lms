...
interface VideoMultiSelectDialogProps {
...
export function VideoMultiSelectDialog({
  options,
  selected,
  onSelectedChange,
  placeholder = "Select videos...",
  isLoading,
}: VideoMultiSelectDialogProps) {
                ...
                {filteredOptions.map((option) => (
                  ...
                  >
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        id={`video-${option.value}`}
                        checked={currentSelected.includes(option.value)}
                        onCheckedChange={() => handleToggle(option.value)}
                      />
                      <Label htmlFor={`video-${option.value}`}>{option.label}</Label>
                    </div>
