...
interface PaperResultsViewProps {
    basePath: string;
}

export function PaperResultsView({ basePath }: PaperResultsViewProps) {
    ...
    const [currentReviewFileUrl, setCurrentReviewFileUrl] = useState<string | undefined>(
        undefined
    );
    ...
    const handleSavePercentage = async (attemptId: string, percentage: number) => {
        try {
            ...
            const token = Cookies.get("token");
            await axios.put(
                `${API_URL}/papers/attempts/${attemptId}/marks`,
                { score: percentage },
                {
                    headers: { Authorization: `Bearer ${token}` },
                }
    ...
    const handleUploadReviewFile = async (attemptId: string, file: File) => {
        try {
            ...
            formData.append("uploadType", "teacher-review");
            formData.append("file", file);

            await axios.post(
                `${API_URL}/papers/attempts/${attemptId}/upload-review`,
                formData,
                {
                    headers: {
                                ...
                                <tbody>
                                    {results.map((result, index) => (
                                        ...
                                        >
                                            ...
                                            <td
                                                className={`p-4 font-bold ${getScoreColor(result.percentage)}`}
                                            >
                                                {result.percentage}%
                                            </td>
